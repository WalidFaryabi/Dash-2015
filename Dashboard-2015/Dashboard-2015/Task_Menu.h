...
typedef enum {MAIN_SCREEN,SYSTEM_MONITOR,TEMP_VOLT,ADJUSTMENT_MAIN,VARIABLE,DEVICE_STATUS,
			  SPEED,TRQ_CALIB,PERSISTENT_MSG,ECU_OPTIONS,LC_HANDLER,ERROR_HANDLER,SNAKE_GAME,
			  STEER_CALIB} EMenuName;
typedef enum {NO_SETTING,TORQUE_SETTING,KERS_SETTING, ECU_P_SETTING, ECU_D_SETTING, 
			  ECU_I_SETTING, ECU_LC_RT_SETTING, ECU_LC_INIT_TORQ_SETTING} EAdjustmentParameter;
			  
typedef enum {JOYSTICK,ACKNOWLEDGE,ROTARY,KERS,DRIVE,LAUNCH_CONTROL,NONE_BTN} EButtonType;
typedef enum {UP,DOWN,LEFT,RIGHT,NAV_DEFAULT} EJoystickDirection;
...
typedef struct ButtonsOnDashboard {
	...
	bool kers_switch_on;
	bool kers_switch_off;
	bool drive_switch_enable;
	bool drive_switch_disable;
	} Buttons;
