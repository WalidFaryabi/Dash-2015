...
int main(void) {
	...
	// Why does the order in which tasks are created matter?
	BaseType_t status;
...
void testSpitask() {
	...
	while(1) {
		...
		uint32_t tbuffer[2] = {0x00010030,0x01010002};
		uint32_t rbuffer[2] = {0};
			
		spi_freeRTOSTranceive(tbuffer,2,NULL,rbuffer);
...
// 	//uint32_t tbuffer[3] = {0x00000008, 0x00000000,0x01000008 };
// 	//uint32_t rbuffer[3];
// 	FT800_Init();
// 	while(1) {
// 		selected = 2;
...
// 		DrawMainMenu(1);
// 		delay_ms(500);
// 		//spi_tranceive(&tbuffer,3,0,0,&rbuffer);
// 		
